# Productivity Prompts

Ready-to-use prompts for busy business owners. Copy, customize, use.

## Tools Covered

- [Dia](https://dia.com) — Email writing
- [Gemini Deep Research](https://gemini.google.com) — Research tasks

## Structure

Every prompt follows the **Input → Context → Output** pattern:

1. **Input** — What you're giving the AI
2. **Context** — Who you are, what you need, constraints
3. **Output** — How you want the response formatted

## Prompts

### Email (Dia)

| Prompt | Use Case |
|--------|----------|
| [Quick Reply](email/quick-reply.md) | Respond to client emails in <2 min |
| [Cold Outreach](email/cold-outreach.md) | First contact to potential client/partner |
| [Follow-Up](email/follow-up.md) | Nudge after no response |
| [Bad News](email/bad-news.md) | Decline or deliver tough info gracefully |
| [Handoff](email/handoff.md) | Introduce two people via email |

### Research (Gemini Deep Research)

| Prompt | Use Case |
|--------|----------|
| [Market Snapshot](research/market-snapshot.md) | "What's happening in [industry] right now?" |
| [Competitor Scan](research/competitor-scan.md) | Understand what competitors are doing |
| [Explain Like I'm Busy](research/explain-like-im-busy.md) | Summarize complex topics in plain language |
| [Find Me Proof](research/find-me-proof.md) | Get statistics/data to back up a claim |
| [Decision Brief](research/decision-brief.md) | Pros/cons analysis for business decisions |

## How to Use

1. Pick a prompt
2. Copy the template
3. Fill in the `[BRACKETED]` sections
4. Paste into Dia or Gemini
5. Review and adjust output

## Contributing

Create a prompt that works? Submit a PR.

**Guidelines:**
- Follow Input → Context → Output format
- Include a filled-in example
- Test before submitting

## License

MIT — use it, share it, build on it.
